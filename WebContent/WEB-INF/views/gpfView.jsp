...
<script type="text/javascript">
	...
	function viewFile(recordNo,sno)
	{
		fileId=recordNo;
		var url='viewFile?department=GPF Ledger Cards&fileId='+recordNo+'&sno='+sno;
...
<div class="form-style-8"  style="margin-bottom: 0px; padding-bottom: 0px;">
	<a href="${pageContext.request.contextPath}/gpfView"><button class="btn btn-primary" style="float: right; margin-right: 10px; cursor: pointer;">Reset</button></a>
    <gpfForm:form action="retrieveFG" id="gpfForm" method="get" modelAttribute="gpfForm">
        <table width="100%" style="border-spacing: 20px; border-top:0px; border-collapse: separate; align: center;">
            <tr>
                ...
                <td>
                	...
                	<gpfForm:input type="date" style="height: 35px;" id="dob" path="dob" list="dobelp" onkeyup="getHelp('dob');"/>
			...
			<tbody>
				<c:forEach items="${records}" var="record">
					<tr>
						...
						<td>${record.series}</td>
						<td><a href="#" style="text-decoration: none;">${record.accountNo}</a></td>
						<td>${record.name}</td>
						<td>${record.dob}</td>
						<td>${record.remarks}</td>
						<td><a href="#" onclick="viewFile('${record.accountNo}','${record.sno}');" style="text-decoration: none;">View</a>&nbsp;&nbsp;</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
