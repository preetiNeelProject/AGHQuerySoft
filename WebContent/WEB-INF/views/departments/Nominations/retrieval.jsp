...
<script type="text/javascript">
	...
	function viewFile(recordNo,sno,right)
	{
		...
		{
			fileId=recordNo;
			var url='viewFile?department=Nominations&fileId='+recordNo+'&sno='+sno;
	...
	{
		if(right=='1')
			window.location="downloadFile?department=Nominations&fileId="+fileId+"&sno="+sno;
		else
	...
	{
		if(right=='1')
			window.location='updateFile?department=Nominations&sno='+sno;
		else
			setContent('You are not authorised to update Files.');
	}
	function trackFile(recordNo)
	{
		window.open("trackRecord?department=Nominations&fileId="+recordNo,"","width=1200,height=700");
	}
...
<p>&nbsp;</p>
<p class="h2" style=" text-align: center; color:#567080;">Retrieve Nominations</p>
<div class="form-style-8"  style="margin-bottom: 0px; padding-bottom: 0px;">
	<a href="${pageContext.request.contextPath}/retrieval?department=Nominations"><button class="btn btn-primary" style="float: right; margin-right: 10px; cursor: pointer;">Reset</button></a>
    <nomForm:form action="retrieveN" id="nomForm" method="get" modelAttribute="nomForm">
        <table width="100%" style="border-spacing: 20px; border-top:0px; border-collapse: separate; align: center;">
            <tr>
                <td>
                	...
                	<nomForm:input style="height: 35px;" id="series" path="series" list="seriesHelp" onkeyup="getHelp('series');"/>
                	<datalist id="seriesHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Account No.:</label><br>
                	<nomForm:input style="height: 35px;" id="accountNo" path="accountNo" list="accountNoHelp" onkeyup="getHelp('accountNo');"/>
                	<datalist id="accountNoHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Name:</label><br>
                	<nomForm:input style="height: 35px;" id="name" path="name" list="nameHelp" onkeyup="getHelp('name');"/>
			...
			<tbody>
				<c:forEach items="${records}" var="record">
					<tr>
						...
						<td>${record.name}</td>
						<td>
							<a href="#" onclick="viewFile('${record.pdfName}','${record.sno}','${view}');" style="text-decoration: none;">View</a>&nbsp;&nbsp;
							<a href="#" onclick="downloadFile('${record.sno}','${record.pdfName}','${download}');" style="text-decoration: none;">Download</a>&nbsp;&nbsp;
							<c:if test="${not empty lg}"><a href="#" onclick="trackFile('${record.pdfName}')" style="text-decoration: none;">Record Audit</a></c:if>
						</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		<input type="hidden" name="department" value="Nominations"/>
