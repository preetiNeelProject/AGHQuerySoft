...
<script type="text/javascript">
	...
	function viewFile(recordNo,sno,right)
	{
		...
		{
			fileId=recordNo;
			var url='viewFile?department=Debit Vouchers&fileId='+recordNo+'&sno='+sno;
	...
	{
		if(right=='1')
			window.location="downloadFile?department=DebitVouchers&fileId="+fileId+"&sno="+sno;
		else
	...
	{
		if(right=='1')
			window.location='updateFile?department=Debit Vouchers&sno='+sno;
		else
			setContent('You are not authorised to update Files.');
	}
	function trackFile(recordNo)
	{
		window.open("trackRecord?department=Debit Vouchers&fileId="+recordNo,"","width=1200,height=700");
	}
...
<p>&nbsp;</p>
<p class="h2" style=" text-align: center; color:#567080;">Retrieve Debit Vouchers</p>
<div class="form-style-8"  style="margin-bottom: 0px; padding-bottom: 0px;">

	<a href="${pageContext.request.contextPath}/retrieval?department=Debit Vouchers"><button class="btn btn-primary" style="float: right; margin-right: 10px; cursor: pointer;">Reset</button></a>
    <dvForm:form action="retrieveD" id="dvForm" method="get" modelAttribute="dvForm">
        <table width="100%" style="border-spacing: 20px; border-top:0px; border-collapse: separate; align: center;">
            <tr>
                <td>
                	...
                	<dvForm:input style="height: 35px;" id="series" path="series" list="seriesHelp" onkeyup="getHelp('series');"/>
                	<datalist id="seriesHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Account No.:</label><br>
                	<dvForm:input style="height: 35px;" id="accountNo" path="accountNo" list="accountNoHelp" onkeyup="getHelp('accountNo');"/>
                	<datalist id="accountNoHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Name:</label><br>
                	<dvForm:input style="height: 35px;" id="name" path="name" list="nameHelp" onkeyup="getHelp('name');"/>
                	<datalist id="nameHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">VR No.:</label><br>
                	<dvForm:input style="height: 35px;" id="vrNo" path="vrNo" list="vrNoHelp" onkeyup="getHelp('vrNo');"/>
                	<datalist id="vrNoHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Date:</label><br>
                	<dvForm:input  style="height: 35px;" id="date" path="date" list="dateHelp" onkeyup="getHelp('date');"/>
                	<datalist id="dateHelp"></datalist>
                </td>
            </tr>
            <tr>
            	<td>
                	<label style="color:#567080;">Amount:</label><br>
                	<dvForm:input style="height: 35px;" id="amount" path="amount" list="amountHelp" onkeyup="getHelp('amount');"/>
                	<datalist id="amountHelp"></datalist>
                </td>
                <td>
                	<label style="color:#567080;">Treasury:</label><br>
                	<dvForm:input style="height: 35px;" id="treasury" path="treasury" list="treasuryHelp" onkeyup="getHelp('treasury');"/>
                	<datalist id="treasuryHelp"></datalist>
                </td>
            </tr>
...
<c:if test="${not empty records}">
	<form action="generateReport" id="reportForm" method="get">
		...
		<input type="hidden" name="department" value="Debit Vouchers"/>
